...
Other names I could have used are Parser and Unmarshaller but to be honest I just went with what popped in my head first. (I eventually changed this name). The other thing design decision was to decide what the function needed to accept and what it needs to return. In this case, it is pretty clear - it receives a string that should conform to the json grammar and returns the object that the string represents according to the JSON grammar.
